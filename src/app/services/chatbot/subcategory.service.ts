...
export class SubcategoryService {
  ...
  getSubcategoryIdentifier(identifier: string): Observable<any> {
    return this.http.get(`http://localhost:3500/getSubcategoryIdentifier/${identifier}`)
    .pipe(
      catchError(er =>{
        console.log(er)
      return er
  ...
  getSubclassesByCategory(identifierCategory: string): Observable<any> {
    return this.http.get(`http://localhost:3500/getSubclassesByCategory/${identifierCategory}`)
      .pipe(
        catchError(error => {
          console.log(error);
          return error;
